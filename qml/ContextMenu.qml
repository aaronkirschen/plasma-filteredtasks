...
PlasmaExtras.Menu {
    ...
    // AbstractTasksModel role enums (Qt::UserRole+1 = 257).
    // These CANNOT be accessed through a var alias like `property var atm: TaskManager.AbstractTasksModel`
    // because QML's var type loses enum resolution at runtime (compiles fine but returns undefined).
    // Values match KDE Plasma 6 / libtaskmanager as of 2026-03.
    ...
    PlasmaExtras.MenuItem {
        ...
        readonly property PlasmaExtras.Menu moreMenu: PlasmaExtras.Menu {
            ...
            PlasmaExtras.MenuItem {
                enabled: menu.visualParent && menu.get(atm.CanSetNoBorder)

                checkable: true
                checked: menu.visualParent && (menu.get(atm.HasNoBorder) ?? false)
